...
var ChatInput = React.createClass({
    ...
    render: function render() {
        ...
        return React.createElement(
            ...
            React.createElement(
                'p',
                { style: { fontSize: '12px', color: '#535353' } },
                text
            ),
            React.createElement(
                'div',
                { className: 'input-group' },
                React.createElement('input', { ref: this.setRef, value: this.state.text, onKeyPress: this.handleKeyPress, maxLength: 140, type: 'text', onInput: this.handleInput, className: 'form-control input-lg', placeholder: 'Type your message here...' }),
                React.createElement(
                    'span',
                    { className: 'input-group-btn' },
                    React.createElement(
                        'button',
                        { className: 'btn btn-warning btn-lg', onClick: this.handleSend },
                        'Send'
                    )
                )
            )
...
var ChatStream = React.createClass({
    ...
    render: function render() {
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement('img', { src: this.props.user.photo, alt: 'User Avatar', className: 'img-circle' })
            ),
            React.createElement(
                'div',
...
module.exports = React.createClass({
    ...
    loadProfileInfo: function loadProfileInfo() {
        var self = this;
        $.get("/users/" + utils.getCookie("accessToken")).done(function (resp) {
            self.setState({
                userData: resp[0]
            });
        }).fail(function () {
    ...
    render: function render() {
        ...
        if (this.state.userData === "loading") {
            ...
            chatUi = React.createElement(
                ...
                React.createElement(ChatView, { userInfo: this.state.userData }),
                React.createElement(UsersView, { userInfo: this.state.userData, onUserUpdate: this.handleUpdate })
            );
        }
